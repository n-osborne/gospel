...
and mod_defs = {
  ...
  unsupported_ocaml_type_env : Ident.t Env.t;
}
...
let empty_defs =
  {
    ...
    exn_env = Env.empty;
    mod_env = Env.empty;
    unsupported_ocaml_type_env = Env.empty;
  }
...
module Lookup_unsupported_ocaml_type = Lookup (struct
  ...
  let env defs = defs.unsupported_ocaml_type_env
  let err id = W.Unbound_type id
end)

let is_unsupported_ocaml_type env id =
  try
...
let add_unsupported_ocaml_type id defs =
  {
    defs with
    unsupported_ocaml_type_env =
      Env.add id.Ident.id_str id defs.unsupported_ocaml_type_env;
  }

let add_unsupported_ocaml_type env id =
  add_def (add_unsupported_ocaml_type id) env
...
let defs_union ~ocaml m1 m2 =
  ...
  let ounion e1 e2 = if ocaml then Env.union choose_snd e1 e2 else e1 in
  {
    ...
    unsupported_ocaml_type_env =
      union m1.unsupported_ocaml_type_env m2.unsupported_ocaml_type_env;
  }
